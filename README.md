...
以下規則列於台灣麻將協會台數計算原文，目前 `score_hand()` 尚未支援：

| 台數 | 規則 | 說明 | 實作難點 |
|------|------|------|----------|
| +1 | 河底撈魚 | 胡別人丟出的最後一張牌 | 需追蹤牌堆剩餘量為 0 時的放槍事件 |
| +1 | 海底撈月 | 自摸牌堆最後一張牌 | 需追蹤 `m.remain == 0` 時的自摸胡 |
| +2 | 平胡 | 五組順子＋一對，胡他人，兩面聽，無字花 | 需分析全手牌能否分解為全順子 |
| +2 | 全求 | 手牌只剩一張單吊，其餘全吃/碰，胡他人牌 | 需偵測閉門手牌僅剩 1 張的狀態 |
| +4 | 地聽 | 配牌後打出第一張牌即聽牌（叫聽後不換牌） | 需追蹤第一巡旗標與首次棄牌聽牌 |
| +5 | 五暗刻 | 手牌含五組刻子（暗刻）且無副露 | 可擴充既有暗刻計數邏輯 |
| +8 | 天聽 | 配牌即聽牌（叫聽後不換牌） | 需在 `main()` 中補花完成後立即偵測 |
| +8 | 字一色 | 全部字牌組成，圈風／自風／三元牌可複合計 | 需偵測 `all_non_bonus` 全為字牌 |
| +16 | 天胡 | 莊家取完牌後即胡（不計門清/不求/天地聽） | 需在補花後、首次棄牌前偵測胡牌 |
| +16 | 人胡 | 第一巡胡他人牌，無吃碰明槓（清 16 台） | 需追蹤「第一巡」與「無副露」旗標 |
| +16 | 地胡 | 閒家首巡第一次摸牌即自摸胡（清 16 台） | 需追蹤閒家首次摸牌旗標 |

> **注意（半求定義差異）**：
> 原文「半求」定義為「手牌只剩一張單吊自摸，其餘全吃/碰 → +1 台」（閉門僅剩 1 張）；
> 現行實作採用較廣泛的定義：「有副露時放槍胡 → +1 台」。
> 原文對應「胡他人」版本稱為「全求 → +2 台」，目前亦未實作。
